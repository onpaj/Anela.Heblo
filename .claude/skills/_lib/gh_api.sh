...
req_paginate() {
  ...
  local hdrfile allfile bodyfile mergedfile
  hdrfile=$(mktemp)
  allfile=$(mktemp)
  bodyfile=$(mktemp)
  mergedfile=$(mktemp)
  ...
  while [[ -n "$url" ]]; do
    ...
    # Pages can exceed jq/bash's single-argument size limit (E2BIG), so
    # merge via files rather than --argjson.
    jq -c -s '.[0] + .[1]' "$allfile" "$bodyfile" > "$mergedfile"
    mv "$mergedfile" "$allfile"
