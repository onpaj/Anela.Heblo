...
req_paginate() {
  ...
  #
  # Pages are combined via jq --slurpfile (reads from disk) rather than
  # --argjson (passed on the command line) — a single page of comments can
  # exceed the kernel's per-argument execve limit (MAX_ARG_STRLEN, 128KiB on
  # Linux), which fails with "Argument list too long" long before the
  # overall ARG_MAX is reached.
  local method="$1" path="$2"
  local url
  url=$(_api_url "$path")
  [[ "$url" == *"?"* ]] && url="${url}&per_page=100" || url="${url}?per_page=100"
  local tmpdir hdrfile allfile pagefile
  tmpdir=$(mktemp -d)
  hdrfile="$tmpdir/headers"
  allfile="$tmpdir/all.json"
  pagefile="$tmpdir/page.json"
  echo '[]' > "$allfile"
  while [[ -n "$url" ]]; do
    ...
    jq -c -n --slurpfile a "$allfile" --slurpfile b "$pagefile" '$a[0] + $b[0]' > "$allfile.tmp"
