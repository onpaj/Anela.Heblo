...
req_paginate() {
  ...
  local all="[]" hdrfile allfile bodyfile
  hdrfile=$(mktemp)
  allfile=$(mktemp)
  bodyfile=$(mktemp)
  ...
  while [[ -n "$url" ]]; do
    curl -sS --max-time 30 -X "$method" \
      ...
      -H "X-GitHub-Api-Version: 2022-11-28" \
      -D "$hdrfile" "$url" > "$bodyfile"
    # Large comment/review threads can exceed Linux's per-argument size cap
    # (MAX_ARG_STRLEN, ~128KB) — --argjson would hand the whole page to jq as
    # a single argv entry and die with "Argument list too long". --slurpfile
    # reads the page from disk instead, so page size is no longer bounded by
    # the OS's argument-vector limit.
