...
req_paginate() {
  ...
  while [[ -n "$url" ]]; do
    ...
    # Pages can be large enough to blow past the sandbox's per-argument exec
    # limit (seen well under the OS ARG_MAX) when passed via --argjson on the
    # command line — read both operands as files instead.
