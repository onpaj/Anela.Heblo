...
req_paginate() {
  ...
  #
  # Accumulates through temp files rather than `jq --argjson`: a large
  # thread's comment body pushed straight onto the command line can exceed
  # this host's ARG_MAX ("Argument list too long"), especially once a big
  # environment eats into the same budget. Files have no such limit.
