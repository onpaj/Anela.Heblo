...
req_paginate() {
  ...
  #
  # Accumulates through temp FILES rather than --argjson shell arguments: a
  # PR with a long comment/review history can push a single page (or the
  # running total) past Linux's per-argument MAX_ARG_STRLEN (128KiB), which
  # fails as `jq: Argument list too long` (exit 126) despite being nowhere
  # near the overall ARG_MAX — jq -s reads the same JSON from stdin/files
