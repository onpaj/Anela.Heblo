...
req_paginate() {
  ...
  #
  # Pages are merged via `jq -s` reading from FILES, never via `--argjson`
  # with the page body inlined as a command-line argument: Linux caps a
  # single argv/environ string at MAX_ARG_STRLEN (128 KiB), well below what
  # one page of PR/issue comments can reach, and blowing past it fails the
  # exec with "Argument list too long" — silently, since the string itself
  # is well under bash's own ARG_MAX. A large single comment (a full review
  # with quoted diffs) is enough to trigger it on the very first page.
