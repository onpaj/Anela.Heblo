...
req_paginate() {
  ...
  #
  # Pages are combined via files/stdin (`jq -s`), never as --argjson
  # arguments: a page's JSON is passed as a single exec argument, and Linux
  # caps any single argv/envp string at MAX_ARG_STRLEN (128KiB) regardless
  # of the total argv budget (getconf ARG_MAX, usually 2MiB) — a busy PR's
  # comment thread (expanded author/app metadata per comment) crosses that
  # 128KiB per-argument ceiling well before the page hits 100 comments,
  # and jq's exec then fails with E2BIG ("Argument list too long").
  ...
  while [[ -n "$url" ]]; do
    ...
    jq -c -s '.[0] + .[1]' "$allfile" "$bodyfile" > "${allfile}.tmp"
