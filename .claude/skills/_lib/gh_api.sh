...
req_paginate() {
  ...
  #
  # Pages are appended to a file (one JSON array per line) rather than
  # threaded through the shell as a growing --argjson string: a PR with a
  # long comment history can accumulate a multi-MB blob, and passing that
  # on jq's command line hits the OS's ARG_MAX ("Argument list too long").
  local method="$1" path="$2"
  local url
  url=$(_api_url "$path")
  [[ "$url" == *"?"* ]] && url="${url}&per_page=100" || url="${url}?per_page=100"
  local hdrfile pagesfile body
  hdrfile=$(mktemp)
  pagesfile=$(mktemp)
  while [[ -n "$url" ]]; do
