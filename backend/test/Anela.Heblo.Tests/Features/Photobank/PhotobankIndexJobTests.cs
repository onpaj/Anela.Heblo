...
public class PhotobankIndexJobTests
{
    ...
    public async Task ExecuteAsync_InsertsNewPhoto_WithRuleTagsApplied()
    {
        ...
        var expectedTag = new Tag { Id = 42, Name = "produkty" };
        ...
        // New photo — first call returns null (not found)
        _repoMock
    ...
    public async Task ExecuteAsync_PersistsDeltaLink_AfterRun()
    {
        ...
        // Assert — the job mutates the root in-place; inspect captured state
    ...
    {
        // Arrange — GetActiveRootsWithDriveAsync already filters inactive; return empty list
        _repoMock
            .Setup(r => r.GetActiveRootsWithDriveAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync([]);
