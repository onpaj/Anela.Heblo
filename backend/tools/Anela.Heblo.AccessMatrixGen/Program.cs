...
foreach (var f in AccessMatrix.Features)
{
    var paths = new List<string>();
    if (f.Path is not null) paths.Add(f.Path);
    if (f.AdditionalPaths is not null) paths.AddRange(f.AdditionalPaths);
    foreach (var p in paths)
        sb.AppendLine($"  \"{p}\": \"{f.Key}.read\",");
}
sb.AppendLine("};");
sb.AppendLine();
