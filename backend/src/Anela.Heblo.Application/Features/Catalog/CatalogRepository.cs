...
// retained for ManufactureHistoryRecord; see ICatalogManufactureSource follow-up
...
public class CatalogRepository : ICatalogRepository
{
    ...
    {
        var dateFrom = _timeProvider.GetUtcNow().Date.AddDays(-1 * _options.Value.ManufactureHistoryDays);
        var dateTo = _timeProvider.GetUtcNow().Date;
        CachedManufactureHistoryData = (await _manufactureSource.GetManufactureHistoryAsync(dateFrom, dateTo, ct)).ToList();
    }
