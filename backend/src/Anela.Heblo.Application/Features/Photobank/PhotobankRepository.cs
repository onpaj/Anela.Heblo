...
namespace Anela.Heblo.Application.Features.Photobank
{
    public class PhotobankRepository : IPhotobankRepository
    {
        ...
        {
            var rows = await _context.PhotobankTags
                .GroupJoin(
                    _context.PhotoTags,
                    t => t.Id,
                    pt => pt.TagId,
                    (t, pts) => new { t, pts })
                .SelectMany(
                    x => x.pts.DefaultIfEmpty(),
                    (x, pt) => new { x.t.Id, x.t.Name, pt })
                .GroupBy(x => new { x.Id, x.Name })
                .Select(g => new
                {
                    g.Key.Id,
                    g.Key.Name,
                    Count = g.Count(x => x.pt != null)
                })
            ...
            return rows.Select(x => new TagCount(x.Id, x.Name, x.Count)).ToList();
        }
