        ...
        public IssuedInvoiceMapping()
        {
            CreateMap<Invoice, IssuedInvoiceDetail>()
                ...
                .AfterMap((i, ii) =>
                {
                    ...
                    {
                        ii.Price = new IssuedInvoices.Price()
                        {
            ...
            CreateMap<InvoiceItem, IssuedInvoiceDetailItem>()
                ...
                .AfterMap(((item, invoiceItem, ctx) =>
                {
                    if (item.ForeignCurrency != null)
                        invoiceItem.ItemPrice = ctx.Mapper.Map<ForeignCurrency, IssuedInvoices.Price>(item.ForeignCurrency);
                    else
                        invoiceItem.ItemPrice = ctx.Mapper.Map<HomeCurrency, IssuedInvoices.Price>(item.HomeCurrency);
            ...
            CreateMap<HomeCurrency, IssuedInvoices.Price>()
                .AfterMap((hc, p) =>
                    {
            ...
            CreateMap<ForeignCurrency, IssuedInvoices.Price>()
                .AfterMap((hc, p) =>
                {
                    p.WithVat = hc.PriceVAT + hc.Price;
                    p.WithoutVat = hc.Price;
            ...
            CreateMap<PartnerIdentity, Customer>()
            ...
            CreateMap<Address, IssuedInvoices.Address>()
            ...
            CreateMap<ShipToAddress, IssuedInvoices.Address>()
                .ForMember(m => m.Company, c => c.MapFrom(f => f.Company))
                .ForMember(m => m.FullName, c => c.MapFrom(f => f.Name))
                .ForMember(m => m.Street, c => c.MapFrom(f => f.Street))
                .ForMember(m => m.Zip, c => c.MapFrom(f => f.Zip))
