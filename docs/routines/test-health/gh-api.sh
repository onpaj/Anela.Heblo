#!/usr/bin/env bash
#
# gh-api.sh — minimal authenticated GitHub REST API helper for the test-health
...
#   docs/routines/test-health/gh-api.sh GET '/search/issues?q=repo:onpaj/Anela.Heblo+label:test-health+in:body+test-signal'
#
#   # Search issues (open AND closed) for a test-signal fingerprint
#   docs/routines/test-health/gh-api.sh find-signal 'test-regress:e2e:catalog:1a2b3c4d'
#
#   # Create an issue (body from a file or stdin)
#   docs/routines/test-health/gh-api.sh create-issue "title" "test-health,test-regression,harness:todo" body.md
#   printf '...' | docs/routines/test-health/gh-api.sh create-issue "title" "test-health,test-regression,harness:todo" -
#
set -euo pipefail
