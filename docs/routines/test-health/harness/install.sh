...
echo
echo "############################################################"
echo "# BEFORE THIS ROUTINE CAN WORK, verify:"
echo "#"
echo "#  1. Labels on onpaj/Anela.Heblo (create manually — this"
echo "#     installer will NOT create them, that is the operator's"
echo "#     call, see docs/routines/test-health/README.md#labels):"
echo "#       test-health   test-infra   test-regression   test-flaky"
echo "#     (harness:todo already exists.)"
echo "#"
echo "#  2. Secrets in ~/harness-root/secrets.env:"
echo "#       RP_API_KEY   RP_ENDPOINT   RP_PROJECT"
echo "#"
echo "# Missing either means silent misfiling (labels) or the routine"
echo "# never running at all (secrets)."
echo "############################################################"
