# Enforce CRON Expression Format Validation in RecurringJobConfiguration Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make `RecurringJobConfiguration` reject a syntactically malformed CRON expression on every write path (constructor, `UpdateConfiguration`, `UpdateCronExpression`), not only on the one path currently guarded by `UpdateRecurringJobCronHandler`.

**Architecture:** Add a private static `ValidateCronFormat(string)` helper to `RecurringJobConfiguration` that checks the trimmed value splits into exactly 5 (standard) or 6 (Quartz-style) whitespace-delimited fields, throwing `System.ComponentModel.DataAnnotations.ValidationException` otherwise — matching the class's existing guard-clause style exactly. Call it from all three write paths, immediately after each method's existing `IsNullOrWhiteSpace(cronExpression)` check. No new files, no new types, no new package references (per arch-review Decision 1 and 2 — this is a structural, not semantic, check, and stays out of `Anela.Heblo.Domain`'s dependency graph).

**Tech Stack:** .NET 8, xUnit, `System.ComponentModel.DataAnnotations.ValidationException` (already used throughout the class).
