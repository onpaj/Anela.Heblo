# Fix broken Architecture Documentation path in CLAUDE.md Implementation Plan

**Goal:** Correct the broken `docs/📘 Architecture Documentation – MVP Work.md` path on line 15 of `CLAUDE.md` so it points to the file's actual location at `docs/architecture/📘 Architecture Documentation – MVP Work.md`.

**Architecture:** No components, services, or runtime behavior are involved. This is a single-line text edit to a Markdown bullet in `CLAUDE.md`'s "Documentation map" section — insert the missing `architecture/` path segment while leaving the emoji, en-dash, filename, and description byte-identical.

**Tech Stack:** None — plain Markdown file edit, verified with shell commands (`find`/`test -f`) and `git diff`.

---

## Context

`CLAUDE.md` instructs readers to consult the documentation map before touching an area of the codebase — "No architectural changes without consulting these first." The very first entry under **Architecture**, the top-level architecture doc, currently points to a non-existent path (`docs/📘 Architecture Documentation – MVP Work.md`), while the file actually lives one directory deeper, at `docs/architecture/📘 Architecture Documentation – MVP Work.md`. Every sibling entry in the same list (`docs/architecture/filesystem.md`, `docs/development/setup.md`, etc.) already resolves correctly — this one bullet is simply missing the `architecture/` path segment.

This has been confirmed directly in the repository:
- `CLAUDE.md:15` currently reads: `` - `docs/📘 Architecture Documentation – MVP Work.md` — modules, data flow, business logic ``
- The file exists at: `./docs/architecture/📘 Architecture Documentation – MVP Work.md` (confirmed via `find . -iname "*Architecture Doc*"`)

The spec, architecture review, and design all agree: this is a one-line path correction with no alternatives to weigh, no components involved, and no further scope. `arch-review.r1.md` sets `Skip Design: true` and proposes no amendments to the spec.

## Requirements traceability

- **FR-1** (`spec.r1.md`): Correct the Architecture Documentation path in `CLAUDE.md`, line 15 — covered by the single task below (edit step + verification step).
