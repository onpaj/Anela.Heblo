# Coverage Verification — ScanPackingOrderHandler Gaps Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a ReportGenerator merge step to both CI workflow files so that coverage from all six test projects is combined into one Cobertura XML before the coverage-gap routine reads it, eliminating the false 32.9% reading for `ScanPackingOrderHandler.cs`.

**Architecture:** After `dotnet test` runs, each test project deposits its own `coverage.cobertura.xml` under `./coverage/`. Without merging, the coverage-gap routine may read only one project's XML and miss lines exercised by other test projects. Adding `dotnet-reportgenerator-globaltool` after the test step merges all per-project XMLs into a single `coverage/merged/Cobertura.xml` that the downstream steps consume.

**Tech Stack:** GitHub Actions YAML, dotnet-reportgenerator-globaltool 5.4.3, Coverlet/XPlat Code Coverage.
...
|------|--------|--------------|
| `.github/workflows/ci-main-branch.yml` | Modify | Insert merge step; update Process and Artifact steps to use merged file |
| `.github/workflows/ci-feature-branch.yml` | Modify | Identical backend-tests changes |
