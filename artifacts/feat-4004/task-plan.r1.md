# Remove Unused `IJournalRepository.GetEntriesByProductAsync` Implementation Plan

**Goal:** Delete the dead `GetEntriesByProductAsync` method from `IJournalRepository`/`JournalRepository` and its six dedicated integration tests, since the same "entries for a product" behavior is already served in production by `SearchEntriesAsync`'s `productCodePrefix` parameter.

**Architecture:** Pure subtractive change inside the Journal module only — no new code, no behavior change, no consumer to migrate. Remove the interface member, its EF Core implementation, the six `[Fact]` tests that exercised it, and (conditionally, after verifying it has no other caller) the now-orphaned `CreateEntryWithFamily` test helper.

**Tech Stack:** .NET 8, EF Core (in-memory provider for integration tests), xUnit, FluentAssertions, Moq.

---

## Before you start

This is a removal-only change with zero production callers, so there is no red/green TDD cycle here — there is no new behavior to drive out with a failing test first. Instead the verification loop is: **confirm the current tests pass → delete the dead code and its tests → confirm the build and the remaining tests still pass.** Do not invent a new test for this task.

Every edit below is given as an exact "find this text, replace with this text" pair. Match the text exactly (including indentation) when locating it in the file — do not rely on the line numbers quoted in the surrounding prose, since they shift after each edit; they are given only so you can locate the right area quickly.
