# SubmitDraftReplyFeedbackHandler Unit Test Coverage Implementation Plan

**Goal:** Add a focused xUnit test suite for `SubmitDraftReplyFeedbackHandler` covering all four execution paths (not-found/wrong-feature, ownership guard, duplicate-submission guard, success), raising the file's line coverage above the 60% threshold with zero production code changes.

**Architecture:** A single new test class, `SubmitDraftReplyFeedbackHandlerTests`, added flat under `backend/test/Anela.Heblo.Tests/Features/Smartsupp/` alongside the existing `GetDraftReplyFeedbackListHandlerTests.cs`, copying its field-initialized-mock + `CreateHandler()` factory pattern. Each of the two collaborators (`IRagInteractionLogRepository`, `ICurrentUserService`) is mocked with Moq; no `DbContext`, no real database.
