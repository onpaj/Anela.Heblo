...
`FinancialAnalysisService.cs` currently builds an identical `FinancialSummaryDto` in three separate methods (`GetHybridWithCurrentMonthAsync`, `GetCachedFinancialOverview`, `GetFinancialOverviewRealTimeAsync`) and maintains two near-duplicate `CreateStockSummary` overloads to support them. This plan extracts a single private `BuildSummary` helper, collapses the two `CreateStockSummary` overloads into one (the `List<MonthlyFinancialDataDto>`-based one), and restructures `GetFinancialOverviewRealTimeAsync` to materialize its DTO list once and reuse it for both `response.Data` and `Summary`. This is a pure internal refactor: no public interface, DTO shape, or observable API/response behavior changes. All existing tests must pass unchanged.

## Architecture

No new components, services, files, or DI registrations. All changes are confined to one existing class:
`backend/src/Anela.Heblo.Application/Features/FinancialOverview/Services/FinancialAnalysisService.cs`

Before:
```
GetHybridWithCurrentMonthAsync      -> inline `new FinancialSummaryDto { ... StockSummary = CreateStockSummary(allData) }`
GetCachedFinancialOverview          -> inline `new FinancialSummaryDto { ... StockSummary = CreateStockSummary(orderedData) }`
GetFinancialOverviewRealTimeAsync   -> inline `new FinancialSummaryDto { ... StockSummary = CreateStockSummary(monthlyData, stockChangesList) }`
CreateStockSummary(List<MonthlyFinancialDataDto>)                                  [kept]
CreateStockSummary(List<MonthlyFinancialData>, List<MonthlyStockChange>)           [removed]
```

After:
```
GetHybridWithCurrentMonthAsync      -> Summary = BuildSummary(allData, includeStockData)
GetCachedFinancialOverview          -> Summary = BuildSummary(orderedData, includeStockData)
GetFinancialOverviewRealTimeAsync   -> Summary = BuildSummary(orderedData, includeStockData)   [orderedData materialized once, reused for Data too]
BuildSummary(List<MonthlyFinancialDataDto>, bool) [NEW private static]
  -> CreateStockSummary(List<MonthlyFinancialDataDto>)  [the one remaining overload]
```

## Tech Stack

- Backend: .NET 8, C#, xUnit (existing test infrastructure — no new test framework or packages).
- Solution file: `Anela.Heblo.sln` (repo root).
- File under change: `backend/src/Anela.Heblo.Application/Features/FinancialOverview/Services/FinancialAnalysisService.cs`.
- Existing tests (must pass unchanged, no modifications):
  - `backend/test/Anela.Heblo.Tests/Application/FinancialOverview/FinancialAnalysisServiceTests.cs`
  - `backend/test/Anela.Heblo.Tests/Application/FinancialOverview/GetFinancialOverviewHandlerTests.cs`
  - `backend/test/Anela.Heblo.Tests/Application/FinancialOverview/FinancialOverviewModuleTests.cs`

## Baseline command

Run this once before making any code changes, from the repository root (`/home/user/worktrees/feature-3493-Arch-Review-Financialoverview-Financialsummarydto`):

```bash
dotnet test backend/test/Anela.Heblo.Tests/Anela.Heblo.Tests.csproj --filter "FullyQualifiedName~FinancialOverview"
```

Expected: all tests in the `FinancialOverview` namespace pass (this covers `FinancialAnalysisServiceTests`, `GetFinancialOverviewHandlerTests`, and `FinancialOverviewModuleTests`). Note the pass count reported at the end (e.g. `Passed! - Failed: 0, Passed: N, Skipped: 0`) — you will compare against this exact count after the refactor.
