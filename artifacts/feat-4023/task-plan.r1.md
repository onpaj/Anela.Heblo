# Fix stale "ProductExportDownload" log label in DownloadFromUrlHandler Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the three hardcoded, stale `"ProductExportDownload"` log-message labels in `DownloadFromUrlHandler` with the accurate `"DownloadFromUrl"` label — logging text only, no behavior change.

**Architecture:** Single-file text edit in `backend/src/Anela.Heblo.Application/Features/FileStorage/UseCases/DownloadFromUrl/DownloadFromUrlHandler.cs`. Three `ILogger` call sites change their message-template literal; no signatures, control flow, structured-logging placeholders, or return values change. No new files, no test file changes are required (existing tests do not assert on log message text).

**Tech Stack:** .NET 8, MediatR, `Microsoft.Extensions.Logging`, xUnit + Moq (verification only, no new test infra needed).
