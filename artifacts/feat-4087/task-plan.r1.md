# IndexDocumentHandler Phase Extraction Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Split `IndexDocumentHandler.Handle`'s 110-line body into four private methods (`TryResolveDuplicateAsync`, `CreateAndPersistDocumentAsync`, `IndexWithErrorHandlingAsync`, `BuildResponse`) so `Handle` reads as a short orchestrator, with zero behavior change.

**Architecture:** Single-file backend refactor, no new classes or interfaces. `Handle` keeps its public signature and delegates in order to the four private methods; `TryResolveDuplicateAsync` returns `IndexDocumentResponse?` (non-null short-circuits `Handle` on either duplicate-detection strategy matching by hash; identity-match instead deletes the stale document and returns `null` so a fresh document gets created). See `arch-review.r1.md` Key Design Decisions for the three implementer choices this plan locks in (single-sourced `useGraphIdentity`, success log placement in `Handle`, shared `BuildResponse` helper).

**Tech Stack:** .NET 8, MediatR request handler, xUnit + Moq (existing test suite is the full behavioral contract; no test changes in scope).
