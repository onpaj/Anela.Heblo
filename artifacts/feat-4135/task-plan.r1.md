# Inject TimeProvider into GetPurchaseStockAnalysisHandler Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Inject `TimeProvider` into `GetPurchaseStockAnalysisHandler` so the default date-range fallback is deterministic and testable, mirroring the existing `CreatePurchaseOrderHandler` pattern.

**Architecture:** No new components or DI registrations — `TimeProvider.System` is already registered as a singleton in `ServiceCollectionExtensions.cs`. This is a single-handler internal refactor: add `TimeProvider` as the last constructor parameter, replace the two `DateTime.UtcNow` reads in `Handle` with one `_timeProvider.GetUtcNow().UtcDateTime` read, and update the handler's test class (built once in its constructor) to inject a mocked `TimeProvider` with a fixed `DateTimeOffset`, matching `CreatePurchaseOrderHandlerTests`.
