# RecurringJobSeeder TimeProvider Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make `RecurringJobSeeder` use the injected `TimeProvider` abstraction instead of `DateTime.UtcNow`, matching the convention every other class in the BackgroundJobs module already follows, and make the resulting audit timestamp exactly assertable in tests.

**Architecture:** Add a `TimeProvider` constructor parameter to `RecurringJobSeeder` (resolved automatically — `TimeProvider.System` is already a DI singleton), compute `now` once per `SeedDefaultConfigurationsAsync` call via `_timeProvider.GetUtcNow().UtcDateTime`, and use it at both existing `DateTime.UtcNow` call sites. Update `RecurringJobSeederTests` to construct the seeder with a `FakeTimeProvider` and assert the exact `LastModifiedAt` value written on both the create and update paths.

**Tech Stack:** .NET 8, `System.TimeProvider` (BCL), `Microsoft.Extensions.Time.Testing.FakeTimeProvider` (already referenced by the test project), xUnit, EF Core InMemory provider.
