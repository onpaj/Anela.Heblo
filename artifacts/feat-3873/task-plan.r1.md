# Replace hand-rolled `apiFetch` in `useCatalogDocuments.ts` Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Remove the `(apiClient as any)` private-field reach-around and hand-declared duplicate DTOs from `frontend/src/api/hooks/useCatalogDocuments.ts`, routing all five hooks through the generated NSwag `ApiClient`'s typed `catalogDocuments_*` methods instead, with zero behavioral change.

**Architecture:** Single-file internal refactor. `useCatalogDocuments.ts`'s `queryFn`/`mutationFn` bodies switch from `apiFetch()` (which reaches into `ApiClient`'s private `baseUrl`/`http` fields) to calling `getAuthenticatedApiClient().catalogDocuments_*(...)` directly, and the six hand-declared local types are deleted in favor of the equivalent generated classes/enum in `frontend/src/api/generated/api-client.ts`. Because two shared display components (`DocumentList.tsx`, `FolderStatusBanner.tsx`) and their tests import `CatalogDocumentDto`/`FolderStatus` **by name from the hook file** (not from the generated client), and the generated versions of those types differ subtly from the hand-rolled ones (`CatalogDocumentDto.modifiedAt`/`sizeBytes`/`name`/`webUrl` become optional and `modifiedAt` becomes `Date` instead of `string`; `FolderStatus` becomes a nominal TypeScript enum instead of a plain string-literal union), those four files also need small, mechanical updates so the project still compiles under `tsconfig.json`'s `strict: true`. This was verified directly against the generated client source and by compiling representative snippets with `tsc --strict` (see Step 0 below) — it is not covered by the spec's FR-4 "four consumers" list (that list only names `MaterialDocumentsTab.tsx`, `PifDocumentsTab.tsx`, `MaterialUploadDialog.tsx`, `PifUploadDialog.tsx`) or by `design.r1.md`'s data-schema table (which incorrectly lists `modifiedAt` as `string`), but is required for `npm run build`'s "no new TypeScript errors" acceptance bar (NFR/FR-4) to actually hold, since `react-scripts build` type-checks the whole `src` program, tests included.

**Tech Stack:** React 18, TypeScript (`strict: true`), `@tanstack/react-query` v5, Jest + React Testing Library (via `react-scripts test`, babel-based — does not type-check), `react-scripts build` (type-checks the full `src` program via its TypeScript integration).
