# Task Plan — Remove orphaned InvoiceClassification statistics dead code (#3523)

## Overview

This is a pure dead-code removal with no behavior change. `GetStatisticsAsync` on
`ClassificationHistoryRepository` is never declared on `IClassificationHistoryRepository`
and is never called anywhere in the codebase — it, its two supporting domain types, their
two DTOs, the AutoMapper registrations for them, and one unused frontend query-key constant
are all orphaned. Because the mapping profile references the domain/DTO types being deleted,
and the repository method references the same domain types, all six deletions/edits are
tightly coupled and must land in a single atomic change to keep the build green at every
intermediate step within the task. This plan is therefore exactly **one task** covering
FR-1 through FR-6. Do not touch `IClassificationHistoryRepository.cs`,
`InvoiceClassificationModule.cs`, `ClassificationStats.tsx`, `InvoiceClassificationPage.tsx`,
or any EF migration/configuration — all out of scope per the architecture review.
