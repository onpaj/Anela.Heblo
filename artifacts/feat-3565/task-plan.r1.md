# Task Plan: Deduplicate the "critical error" business rule in IssuedInvoice

Source artifacts: `artifacts/feat-3565/spec.r1.md`, `artifacts/feat-3565/arch-review.r1.md` (design skipped — backend-only).

This is a single small, well-scoped refactor (entity + repository + regression test are tightly coupled and must land together — splitting them would leave an intermediate state where the two definitions still exist or the test can't compile). One task covers the whole change.
