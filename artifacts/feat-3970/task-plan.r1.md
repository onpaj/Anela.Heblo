# Remove Hangfire AutomaticRetry Leak from ProductPairingDqtJob Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Remove the Hangfire `using` import and `[AutomaticRetry]` attribute from `ProductPairingDqtJob` so it matches its three DataQuality sibling jobs, which reference no Hangfire types at all.

**Architecture:** Single-file, attribute-only edit in the Application layer. No new types, no registration-path changes — `HangfireJobRegistrationHelper` already registers/schedules any `IRecurringJob` reflectively, independent of whether `[AutomaticRetry]` decorates the method. The job falls back to Hangfire's built-in default retry policy (10 attempts), the same policy its three siblings already run under.

**Tech Stack:** .NET 8, xUnit, Moq, Hangfire (attribute removed, package reference untouched).

Full plan also saved to `docs/superpowers/plans/2026-08-29-remove-hangfire-attribute-productpairingdqtjob.md`.
