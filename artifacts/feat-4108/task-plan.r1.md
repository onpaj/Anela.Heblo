# Journal Default Sort Key ("entryDate") Warning Fix Implementation Plan

**Goal:** Stop `JournalRepository.ApplySort()` from logging a spurious "Unknown sort key" warning on every default-sorted (`sortBy = "entryDate"`) Journal request, without changing sort behavior.
**Architecture:** Single-file, single-method fix in the persistence layer — add an explicit `"entrydate"` arm to the existing `switch` expression in `JournalRepository.ApplySort()` (`backend/src/Anela.Heblo.Persistence/Journal/JournalRepository.cs`) that delegates to the existing `ApplyDefaultSort(query, ascending)` helper with no logging, placed before the `_` default arm that calls `ApplyDefaultSortWithWarning`. No new types, no public contract changes, no other files touched besides the corresponding test file.
**Tech Stack:** .NET 8, xUnit, Moq, FluentAssertions, EF Core InMemory provider (existing `JournalRepositoryIntegrationTests` test harness).
