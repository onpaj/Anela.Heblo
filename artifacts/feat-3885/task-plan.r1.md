# Fix CatalogDataRefreshService Cache Isolation Violations Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Stop `RefreshManufactureDifficultySettingsData` (single-product branch) and `RefreshManufactureCostData` in `CatalogDataRefreshService` from mutating live cached objects in place; route both through clone-before-mutate + `ReplaceCacheAtomicallyAsync`/`SetManufactureDifficultySettingsData`, matching the pattern `CatalogMergeService.Merge()` already uses since #3827.

**Architecture:** Both methods currently obtain a reference to a live cached collection (`_cacheStore.GetManufactureDifficultySettingsData()` / `_cacheStore.GetCatalogData()` / `_cacheStore.TryGetCurrent()`) and write into it or its elements directly. The fix builds a **new** dictionary/list — cloning only the elements that actually change (`CatalogAggregate.Clone()`, already deep-copies `ManufactureDifficultySettings`) — and installs it via the existing setter (`SetManufactureDifficultySettingsData`) or the existing atomic swap (`ReplaceCacheAtomicallyAsync`). No public interfaces change.

**Tech Stack:** .NET 8, C#, xUnit, FluentAssertions, Moq, `Microsoft.Extensions.Caching.Memory.MemoryCache`.
