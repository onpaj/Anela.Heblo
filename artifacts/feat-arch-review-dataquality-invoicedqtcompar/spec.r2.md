...
> **Note on `answers.r1.md`:** The provided `answers.r1.md` answers questions about a different feature (BankStatement filtering: `ImportResult`, `Account`, date filters). None of its answers apply to the four Open Questions in this spec's prior revision. The Open Questions below are therefore carried over unchanged.
...
5. **Mismatched `answers.r1.md`.** The supplied answer revision addresses a different feature (BankStatement `ImportResult` / `Account` / date filtering) and does not resolve any of the four questions above. Please supply answers targeting questions 1–4 in the next revision.
