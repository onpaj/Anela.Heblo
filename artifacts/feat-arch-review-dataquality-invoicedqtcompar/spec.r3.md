...
> **Note on supplied answer revisions (`answers.r1.md`, `answers.r2.md`).** Both supplied answer revisions address a different feature (BankStatement filtering: `ImportResult`, `Account`, date filters). None of their answers map onto the four Open Questions in this spec's prior revisions. The Open Questions below are therefore carried over unchanged.
...
5. **Mismatched answer revisions.** Both `answers.r1.md` and `answers.r2.md` address a different feature (BankStatement `ImportResult` / `Account` / date filtering) and do not resolve any of the four questions above. Please supply answers targeting questions 1–4 in the next revision.
