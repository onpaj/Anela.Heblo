# OrgChartService Success-Path Test Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add one success-path unit test to `OrgChartServiceTests` that proves valid external JSON deserializes correctly into `OrgChartResponse`, including nested `PositionDto`/`EmployeeDto` fields, closing the coverage gap identified in issue #4156.

**Architecture:** Test-only change, no production code touched. The new test reuses the existing `StubHttpMessageHandler.Returns(HttpStatusCode, string)`, `CreateService(HttpMessageHandler)`, and `VerifyNoErrorLog()` private helpers already present in `OrgChartServiceTests.cs`, following the exact AAA/FluentAssertions pattern of the four existing error-path tests in that file.

**Tech Stack:** .NET 8, xUnit, FluentAssertions, Moq (all already referenced by the `Anela.Heblo.Tests` project — no new dependencies).

---

## Scope Check

Single subsystem (OrgChart backend test coverage), single file touched, one new test method. Does not need to be split into multiple plans.

## File Structure

Only one file changes:
- **Modify:** `backend/test/Anela.Heblo.Tests/Features/OrgChart/OrgChartServiceTests.cs` — add one `[Fact]` test method. No new files are created; no existing method signatures change.

No production files change (`OrgChartService.cs`, `OrgChartResponse.cs`, `OrganizationDto.cs`, `PositionDto.cs`, `EmployeeDto.cs`, `BaseResponse.cs` are all read-only references for this plan).
