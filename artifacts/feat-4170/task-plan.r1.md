# Coverage: DailyInvoiceImportJobBase Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Close the coverage gap on `DailyInvoiceImportJobBase.ExecuteAsync` by adding unit tests for its three unexercised paths: the disabled-job short-circuit, the partial-failure warning branch, and the exception-rethrow branch.

**Architecture:** One new test file, `DailyInvoiceImportJobBaseTests.cs`, with a private nested `TestDailyInvoiceImportJob` test-double subclass (mirroring the existing `BankImportJobBaseTests.TestBankImportJob` convention) to instantiate the abstract base class under test. No production code changes. Note: because the behaviors under test are already correctly implemented (this is a coverage-only task, not new-feature TDD), each test step below is verified by running it and confirming it **passes** immediately — there is no red/green cycle against production code, but each step still runs the test before moving on to the next, so a broken assumption is caught immediately rather than at the end.

**Tech Stack:** xUnit, Moq, FluentAssertions, `Microsoft.Extensions.Logging.Abstractions` (`NullLoggerFactory`) — all already referenced by `Anela.Heblo.Tests`.
