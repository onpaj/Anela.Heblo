# Task Plan: RecurringJobSeeder must not overwrite audit fields when nothing changed (feat-4318)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Stop `RecurringJobSeeder.SeedDefaultConfigurationsAsync` from stamping `LastModifiedAt`/`LastModifiedBy` on every application restart when none of the developer-owned fields (`DisplayName`, `Description`, `TimeZoneId`) actually changed, so an admin's real CRON/enable-disable audit trail is preserved across restarts.

**Architecture:** Add a private static `HasSeededFieldsChanged(existing, config)` helper to `RecurringJobSeeder` and guard the existing `existing.UpdateConfiguration(...)` / `_repository.UpdateAsync(...)` call with it, inside the `existing != null` branch of the seeding loop. No new files, no interface changes, no migration. One existing test that currently encodes the buggy behavior must be corrected as part of the same change, and one new test added for the "nothing changed → audit fields preserved" case.

**Tech Stack:** .NET 8, C#, xUnit, `Microsoft.Extensions.Time.Testing.FakeTimeProvider`, EF Core InMemory provider (existing test infra, unchanged).
...
|------|----------------|--------|
| `backend/src/Anela.Heblo.Application/Features/BackgroundJobs/Services/RecurringJobSeeder.cs` | Syncs developer-owned fields from code metadata into DB rows at startup | Add `HasSeededFieldsChanged` private static helper; guard the update call in the `existing != null` branch |
| `backend/test/Anela.Heblo.Tests/Features/BackgroundJobs/RecurringJobSeederTests.cs` | Unit tests for the seeder | Fix `SeedDefaultConfigurationsAsync_WhenConfigurationExists_SetsLastModifiedByToSystem` (currently asserts the buggy behavior); add `SeedDefaultConfigurationsAsync_WhenNothingChanged_PreservesLastModifiedAtAndBy` |

No new files. No NuGet packages. No DB migration. No frontend changes.
