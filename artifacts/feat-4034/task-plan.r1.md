# Task Plan: Remove redundant combined `Task.WhenAll` in `CalculateMonthlyStockChangeAsync`

## Overview
Single-line dead-code deletion in `FinancialOverviewStockValueAdapter.CalculateMonthlyStockChangeAsync`: remove a `Task.WhenAll` await whose result is never used. No behavior, performance, or public-contract change. One task covers the entire change.
