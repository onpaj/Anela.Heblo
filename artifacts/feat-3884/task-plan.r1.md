# Gate E2ETestController.GetEnvironmentInfo Behind Environment Check Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add the same "Staging or Development only" environment guard that `CreateE2ESession`, `GetAuthStatus`, and `GetE2EApp` already carry to `E2ETestController.GetEnvironmentInfo`, so it returns `404 NotFound` instead of leaking environment info anonymously in Production.

**Architecture:** Single guard clause added at the top of `GetEnvironmentInfo`'s body, copied verbatim (condition, log-free, response shape) from the three sibling actions in the same file. No new dependencies — `IWebHostEnvironment _environment` is already injected into the controller. No `[Authorize]` attribute added (see arch-review.r1.md Decision 1 — this endpoint must stay anonymously callable in Staging/Development, matching `CreateE2ESession`).

**Tech Stack:** .NET 8, ASP.NET Core MVC, xUnit, FluentAssertions, Moq.
