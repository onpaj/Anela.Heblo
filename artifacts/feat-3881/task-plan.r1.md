# Implementation Plan: Move the `JobStorage` DI registration out of `DashboardModule`

Source docs: `artifacts/feat-3881/spec.r1.md`, `artifacts/feat-3881/arch-review.r1.md`, `artifacts/feat-3881/design.r1.md`.

## Overview

`DashboardModule.AddDashboardModule()` (`backend/src/Anela.Heblo.Application/Features/Dashboard/DashboardModule.cs:19-20`) registers the backend's only `Hangfire.JobStorage` singleton, even though nothing in Dashboard's own code consumes it. The real consumers, `HangfireBackgroundWorker` and `HangfireFailedJobCounter`, live in `backend/src/Anela.Heblo.API/Infrastructure/Hangfire/` and are registered together in `ServiceCollectionExtensions.AddHangfireServices` (`backend/src/Anela.Heblo.API/Extensions/ServiceCollectionExtensions.cs`). If `AddDashboardModule()` is ever skipped or removed, those two adapters fail DI resolution at startup with no discoverable root cause.

This plan relocates the single registration line from `DashboardModule.AddDashboardModule()` into `AddHangfireServices`, placed immediately before the adapter registrations that need it, and adds a regression test guarding against the binding drifting back into `DashboardModule` (or being lost entirely). One task, fully self-contained.

All commands below assume the working directory is the repository root, which contains `Anela.Heblo.sln`.
