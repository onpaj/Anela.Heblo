...
- `backend/test/Anela.Heblo.Tests/Features/Invoices/Infrastructure/InvoiceImportStatisticsSourceAdapterTests.cs` — this file has exactly one reference to `IsBelowThreshold`: line 73, `result[0].IsBelowThreshold.Should().BeFalse();`, inside `GetDailyCountsAsync_InvoiceDateBranch_ReturnsCountsGroupedByDay`. Delete that single assertion line; no other changes to this file are needed. (The three `IsBelowThreshold = false` initializers inside `InvoiceImportStatisticsSourceAdapter.cs` itself are the production code handled under FR-2, not this test file.) The assertion has nothing left to check once the Domain property and the adapter's dummy-value assignment are both removed — it was only ever verifying the adapter's known dummy value, per the `IInvoiceImportStatisticsSource` XML doc ("the consumer decides thresholds"). The surrounding `Date`/`Count` assertions in that test, and all other tests in the file, are untouched and continue to verify gap-filling and grouping behavior exactly as before.
...
- `InvoiceImportStatisticsSourceAdapterTests.cs` no longer contains the `result[0].IsBelowThreshold.Should().BeFalse();` assertion (line ~73), and no replacement assertion on a threshold flag is added.
...
The rename is propagated verbatim through the two frontend files that reference the type name — no aliasing is introduced:

- `frontend/src/api/hooks/useInvoiceImportStatistics.ts` — imports and re-exports `DailyInvoiceCount` from `../generated/api-client`. Rename the import and re-export to `DailyInvoiceCountDto`.
- `frontend/src/components/charts/InvoiceImportChart.tsx` — imports `DailyInvoiceCount` from `'../../api/hooks/useInvoiceImportStatistics'` for its `InvoiceImportChartProps.data` type. Rename this import/type reference to `DailyInvoiceCountDto`; `.map()` usage and field access are unaffected since only the type name changes, not the field shape.
- Re-run frontend codegen (per `docs/development/api-client-generation.md`) as part of the build so `api-client.ts` reflects the new backend type name; do not hand-edit the generated file.

This approach (option (a) — full rename, no local alias) was chosen over introducing a `DailyInvoiceCount` alias because every other hook in `frontend/src/api/hooks/*.ts` (`useDashboard.ts`, `useIssuedInvoices.ts`, `useArticles.ts`, `useBankStatements.ts`, `useMaterialContainers.ts`, `useInvoiceClassification.ts`, `useRecurringJobs.ts`) imports and re-exports generated-client type names verbatim with zero aliasing precedent, and the blast radius is trivially small (only these two files reference the type name).
...
- `npm run lint` passes.
- No local type alias (e.g. `export type DailyInvoiceCount = DailyInvoiceCountDto`) is introduced anywhere in the frontend; all references use `DailyInvoiceCountDto` directly.
...
The only visible change is the OpenAPI schema component name for the array item type, from `DailyInvoiceCount` to `DailyInvoiceCountDto`, which flows into the generated TypeScript client's exported class name (see FR-4). Frontend code references this renamed type directly with no alias.
