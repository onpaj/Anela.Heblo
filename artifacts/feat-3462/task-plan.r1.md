# Fix AddAzurePrintQueueSink Phantom Singleton Registration - Implementation Plan

**Goal:** Split `AzureAdapterModule.AddAzurePrintQueueSink` into an infrastructure-only registration method and a full method that also binds the non-keyed `IPrintQueueSink` singleton, and make the `"Combined"` print-sink mode call only the infrastructure method, so no phantom non-keyed `AzureBlobPrintQueueSink` singleton registration lingers alongside the `CombinedPrintQueueSink` factory.

**Architecture:** `AzureAdapterModule.cs` gains `AddAzurePrintQueueSinkInfrastructure` (registers `BlobContainerClient` + `AzureBlobPrintQueueSink` as a concrete singleton, no `IPrintQueueSink` binding) while `AddAzurePrintQueueSink` becomes a thin wrapper that calls it and adds the non-keyed `IPrintQueueSink` singleton binding via a factory delegate to the same shared instance. `ServiceCollectionExtensions.AddPrintQueueSink`'s `"Combined"` case switches to the infrastructure-only method and its keyed `"azure"` registration is promoted to `AddKeyedSingleton` (factory-backed, same shared instance) so both `"AzureBlob"` and `"Combined"` modes resolve the identical `AzureBlobPrintQueueSink` instance without double-registering `IPrintQueueSink`.

**Tech Stack:** .NET 8, ASP.NET Core DI
