# Remove ProductMarginSegmentDto Backward-Compatibility Aliases Implementation Plan

**Goal:** Delete the six dead backward-compatibility alias properties (`ProductCode`, `ProductName`, `MarginPerPiece`, `SellingPriceWithoutVat`, `MaterialCosts`, `LaborCosts`) from `ProductMarginSegmentDto`, regenerate the NSwag-generated TypeScript client so the six aliases disappear from the wire contract, and rename the stale alias-named fields in the `ProductMarginSummary.test.tsx` `productSegments` fixture to their canonical names — so the DTO exposes exactly one name per concept, front and back.

**Architecture:** Single atomic backend + frontend change, no new components. The producer (`MonthlyBreakdownGenerator.GenerateMonthlySegments`) already writes only canonical fields; the sole test reference (`GetProductMarginSummaryHandlerTests.cs:215`) builds an empty `List<ProductMarginSegmentDto>()` and touches no property; the sole frontend production consumer (`ProductMarginSummary.tsx:245-250`) already reads only canonical `average*`/`groupKey`/`displayName` fields. Only the Jest fixture in `ProductMarginSummary.test.tsx` (lines 32-57) still uses the old alias field names as object-literal keys — renamed here for consistency (not required for tests to pass, since the fixture is `any`-cast and the tooltip callback that reads it is never invoked under the mocked `Chart` component). This mirrors the already-completed sibling cleanup of `TopProductDto`'s two aliases in `docs/superpowers/plans/2026-06-10-remove-topproductdto-shims.md`.

**Tech Stack:** .NET 8, C# (`Anela.Heblo.Application.Features.Analytics.Contracts`), NSwag (OpenAPI client generation via MSBuild target), React + TypeScript, Jest, MSBuild.

**Scope guardrails (read before editing):**
- **Do NOT** modify `MarginCalculator.cs`, `ReportBuilderService.cs`, `ProductFilterService.cs`, `GetProductMarginAnalysis*.cs`, `ProductMarginSummaryDto.cs`, `TopProductDto.cs` (already cleaned up separately), `MonthlyProductMarginDto.cs`, or `MonthlyBreakdownGenerator.cs`. None of these need to change.
- **Do NOT** hand-edit `frontend/src/api/generated/api-client.ts`. It is regenerated by an MSBuild target only.
- **Do NOT** touch the `monthlyData[0].topProducts` fixture block (already using canonical names) or any other test file — only the `productSegments` array inside `mockData` in `ProductMarginSummary.test.tsx` changes.
- **Note on tooling docs:** `docs/development/api-client-generation.md` describes a frontend `npm run generate-client` / `prebuild` script. That script does **not** currently exist in `frontend/package.json` (verified). The actual, working regeneration command in this repo is the MSBuild target below — use it, not the npm script.
