# Remove Dead result.Failed Write in MarketingInvoiceImportService Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Delete the dead `result.Failed += stagedCount;` write (and its orphaned comment) from the catch block around the final `SaveChangesAsync` call in `MarketingInvoiceImportService.ImportAsync`, with no behavior change.

**Architecture:** No architectural change — a two-line deletion inside one existing method's catch block. See `spec.r1.md`, `arch-review.r1.md`, and `design.r1.md` for full context; this plan implements FR-1 and FR-2 of the spec.

**Tech Stack:** .NET 8, xUnit, Moq (existing test project conventions).
