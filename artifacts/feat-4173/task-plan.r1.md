# CriticalGiftPackagesTile Error-Shape Test Coverage Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add unit tests that lock in the JSON response shape of `CriticalGiftPackagesTile.LoadDataAsync`'s two currently-untested error branches (`!response.Success` and the outer `catch (Exception)`), closing GitHub issue #4173's 0.0% line-coverage gap.

**Architecture:** Add one new xUnit test class, `CriticalGiftPackagesTileTests`, at `backend/test/Anela.Heblo.Tests/Features/Logistics/DashboardTiles/CriticalGiftPackagesTileTests.cs`, mirroring the existing `Features/{Module}/DashboardTiles/{Tile}Tests.cs` convention (e.g. `LowStockEfficiencyTileTests`, `WeatherForecastTileTests`). No production code changes — `CriticalGiftPackagesTile.cs` already implements both branches correctly per the issue; the task is to characterize/pin down that existing behavior with tests. Because the behavior already exists and is already correct, this plan's "run to verify it fails" steps intentionally verify a **compile failure** (test file doesn't exist yet) rather than a runtime assertion failure — once the file compiles, both tests are expected to pass immediately, since they exercise already-correct production code.

**Tech Stack:** xUnit, Moq, FluentAssertions, `System.Text.Json` (`JsonSerializer` + `JsonDocument`) — all already used identically in `LowStockEfficiencyTileTests.cs` and `WeatherForecastTileTests.cs`.
