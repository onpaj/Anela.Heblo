# Standardize PurchaseStockAnalysisController Validation Error Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make `PurchaseStockAnalysisController.GetStockAnalysis` return the project's standardized `BaseResponse`-derived validation error envelope instead of raw ASP.NET Core `ModelState`, matching `PurchaseOrdersController`'s pattern.

**Architecture:** One-line call-site substitution — replace `BadRequest(ModelState)` with `BadRequest(ErrorResponseHelper.CreateValidationError<GetPurchaseStockAnalysisResponse>())`. No new types, no data model changes; `GetPurchaseStockAnalysisResponse` already extends `BaseResponse`. Covered by a new controller-level unit test that exercises the invalid-`ModelState` branch directly, mirroring the existing `PurchaseOrdersControllerErrorTests.cs` pattern.

**Tech Stack:** .NET 8, ASP.NET Core MVC, MediatR, xUnit.
