# Task Plan: DataQuality-owned invoice snapshot contracts (feat-3968)

`IInvoiceShoptetSource` and `IInvoiceErpClient` — two consumer contracts owned by the DataQuality
module (`backend/src/Anela.Heblo.Application/Features/DataQuality/Contracts/`) — currently leak
Invoices-module domain types (`IssuedInvoiceDetailBatch`, `IssuedInvoiceSourceQuery`,
`IssuedInvoiceDetail`, `IssuedInvoiceDetailItem`, `InvoicePrice`) through their method signatures, and
`InvoiceDqtComparer` consumes those types directly. This plan introduces three DataQuality-owned
snapshot types (`DqtInvoiceSnapshot`, `DqtInvoiceItem`, `DqtInvoiceSourceQuery`), moves the
Invoices→DataQuality field mapping into the two existing provider-side adapters
(`InvoiceShoptetSourceAdapter`, `InvoiceErpClientAdapter`), rewrites `InvoiceDqtComparer` to consume
only the new types, and finally closes the `DataQuality -> Invoices`
`ModuleBoundariesTests.cs` allowlist to a hard, zero-tolerance CI gate. It is a pure backend
refactor — no HTTP/controller surface, no MediatR contract, no DB schema, and no frontend code is
touched.

The three tasks below are **sequential, not independent**: task 2 requires the types and mapper
built in task 1; task 3 requires the boundary violations to already be fixed by task 2 (its
acceptance check is "the reflection scan finds zero actual references," which is only true once
task 2 has landed). Do them in order: `add-dqt-invoice-snapshot-contracts-and-mapper` →
`route-invoice-dqt-comparer-through-dqt-contracts` → `close-dataquality-invoices-boundary-allowlist`.
