# Journal `Title` Required Annotation Implementation Plan

**Goal:** Add the missing `[Required]` data annotation to `Title` on `CreateJournalEntryRequest` and `UpdateJournalEntryRequest` so the generated OpenAPI spec and TypeScript client correctly describe `title` as a required property, matching the existing server-side/domain/database validation.

**Architecture:** Two one-line additions to existing DTO classes in `Features/Journal/Contracts/` — no new files, no handler/domain/controller/frontend changes. `System.ComponentModel.DataAnnotations` is already imported in both files. The OpenAPI spec and TypeScript client regenerate automatically as part of the normal build (per `docs/development/api-client-generation.md`); this plan does not hand-edit the generated client.

**Tech Stack:** .NET 8 (C#), `System.ComponentModel.DataAnnotations`, Swashbuckle/NSwag (build-time OpenAPI + TS client generation), xUnit.
