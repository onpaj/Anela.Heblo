# Null-guard McpProductNotFoundTelemetryFilter Implementation Plan

**Goal:** Stop `McpProductNotFoundTelemetryFilter.Process` from throwing `NullReferenceException` when an `ExceptionTelemetry` item has a `null` `Message`, and add a regression test that locks the fix in.

**Architecture:** No architectural change. `McpProductNotFoundTelemetryFilter` is one link in the Application Insights `ITelemetryProcessor` chain (`_next.Process(...)` passthrough pattern, mirroring the sibling `AzureBlobConflictTelemetryFilter`). The single unguarded `exc.Message.Contains(...)` expression in `Process` becomes null-safe (`exc.Message?.Contains(...) == true`), so an `ExceptionTelemetry` with `Message == null` simply falls through to the existing non-matching path (`_next.Process(item)`) instead of throwing. No new branch, no new method, no signature change.

**Tech Stack:** .NET 8, `Microsoft.ApplicationInsights` SDK (`ITelemetryProcessor`, `ExceptionTelemetry`, `TraceTelemetry`), xUnit, Moq, FluentAssertions.
