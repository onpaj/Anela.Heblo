...
The `ExpeditionList:BlobConnectionString` key (defined in `appsettings.json:522–532`) is owned by `ExpeditionList` (see `ExpeditionList/PrintPickingListOptions.cs`). If ExpeditionList renames or restructures its config, the blob storage service silently falls back to the development storage emulator (`UseDevelopmentStorage=true`) with no warning, which could result in data being written to the wrong location in production.

This was filed by the daily arch-review routine on 2026-06-05.
...
- The new key is also provisioned in Azure Key Vault for staging (and production, if used) using the project's standard `--` separator: `FileStorage--BlobConnectionString`.
...
- `ExpeditionList` continues to read its own `ExpeditionList:BlobConnectionString` key without modification (the key remains owned by ExpeditionList for its existing usage, if any — see Open Questions).
...
**Acceptance criteria:**
- `docs/architecture/environments.md` (or the appropriate environment doc) mentions the `FileStorage:BlobConnectionString` key and its Key Vault counterpart.
- Any deployment scripts, `azure-pipelines.yml`/GitHub Actions workflows, or local-dev setup scripts that reference `ExpeditionList:BlobConnectionString` for the purpose of FileStorage are updated.
- A short migration note is added to the staging deployment runbook (or equivalent) describing how the key was rolled out so on-call engineers can trace it.
...
### NFR-2: Security
- The new `FileStorage:BlobConnectionString` secret must be stored in Azure Key Vault for staging/production deployments per the project rule: *"All secrets go to Azure Key Vault, never to Web App environment variables."*
- The legacy `ExpeditionList:BlobConnectionString` Key Vault secret should remain in place until verified unused by ExpeditionList itself, then removed in a follow-up cleanup (tracked in Open Questions).
...
### NFR-3: Backward compatibility / rollout safety
- The deployment must be safe to roll forward and back during the rollout window. Either:
  - (a) deploy the Key Vault secret first, then the code change, so the new key is always resolvable when the new code runs; or
  - (b) keep a temporary fallback that reads `FileStorage:BlobConnectionString` first and falls back to `ExpeditionList:BlobConnectionString` with a warning log, removed in a follow-up PR.
- The chosen approach must be stated in the PR description.
...
## Dependencies
- Azure Key Vault (`kv-heblo-stg`) for secret storage.
...
## Out of Scope
- Refactoring `ExpeditionList`'s own use of `ExpeditionList:BlobConnectionString`. If ExpeditionList writes to or reads from blob storage itself, that work continues unchanged.
- Removing the legacy `ExpeditionList:BlobConnectionString` Key Vault secret. That cleanup happens only after confirming no consumer remains (see Open Questions).
...
### Q1: Does ExpeditionList itself still consume `ExpeditionList:BlobConnectionString`?
The brief notes the key is "owned by" ExpeditionList based on `PrintPickingListOptions.cs`, but does not confirm whether ExpeditionList currently *reads* it for its own purposes. If it does, the key stays as-is and we only add the new FileStorage key. If it does not (i.e., the key existed solely for FileStorage to piggy-back on), the legacy key and its Key Vault secret should be removed in a follow-up cleanup PR.

**Assumption made:** the key is retained in `ExpeditionList` for now; cleanup is deferred to a follow-up.

### Q2: Rollout strategy — staged fallback or hard cutover?
NFR-3 lists two safe rollout options. Preference is the **hard cutover with Key Vault provisioned first** (option a) because it keeps the code change minimal and avoids leaving a stale fallback in the tree. Confirm with the deploying engineer before merge.

**Assumption made:** option (a) — Key Vault secret created before the code change is deployed.

### Q3: Is there a production environment for this app?
The project notes mention staging (`kv-heblo-stg`) explicitly but not production. FR-1 and NFR-2 assume production may exist. If it does, the new secret must also be provisioned in the production Key Vault before the production deployment.
