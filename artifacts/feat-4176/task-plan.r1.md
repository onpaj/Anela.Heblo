# GetLeafletGenerationHandler Not-Found Test Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a unit test proving `GetLeafletGenerationHandler` returns `ErrorCodes.LeafletFeedbackNotFound` with all data fields at their defaults when the repository has no matching record.

**Architecture:** Single new xUnit test class in `backend/test/Anela.Heblo.Tests/Features/Leaflet/UseCases/`, mocking `ILeafletGenerationRepository` with Moq and asserting with FluentAssertions, matching the established 1-file-per-handler convention already used by sibling tests in that directory (e.g. `GetLeafletChunkDetailHandlerTests.cs`). No production code changes.

**Tech Stack:** .NET 8, xUnit, Moq, FluentAssertions (all already referenced by the `Anela.Heblo.Tests` project).

**Note on TDD framing:** This is a coverage-gap task, not new-feature TDD — `GetLeafletGenerationHandler`'s not-found branch already exists and is already correct (verified during analysis/architecture: `backend/src/.../GetLeafletGenerationHandler.cs` lines 21-23). The "red" step below therefore isn't "fails because the feature doesn't exist" — it's "fails because the test file doesn't exist yet, i.e. doesn't compile" — and the "green" step is expected to pass on the first correct implementation, since no handler code needs to change.
