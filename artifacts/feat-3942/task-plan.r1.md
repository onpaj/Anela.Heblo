# useSubmitLeafletFeedbackMutation Test Coverage Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add unit tests for `useSubmitLeafletFeedbackMutation`'s `mutationFn` covering the 409 already-submitted path, the non-ok/non-409 throw path, and the ok/parsed-JSON path, with no production code changes.

**Architecture:** Single new Jest/RTL test file added to `frontend/src/api/hooks/__tests__/`, mocking `getAuthenticatedApiClient` from `../../client` exactly as `useBoxFill.test.ts` does for sibling mutation hooks, and driving the hook via `renderHook` + `mutateAsync`.

**Tech Stack:** Jest, `@testing-library/react` (`renderHook`, `waitFor`), `@tanstack/react-query` (`QueryClient`, `QueryClientProvider`), TypeScript, React 18.
