# Resolve ReplayedBy Identity Inside ReplayWebhookEventHandler Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Move identity resolution for the Smartsupp webhook-replay audit field out of `SmartsuppWebhookAuditController` and into `ReplayWebhookEventHandler` via `ICurrentUserService`, closing the ADR-005 violation where the controller reads `User.Identity?.Name` and stamps it onto a client-settable `ReplayWebhookEventRequest.ReplayedBy` field.

**Architecture:** `ReplayWebhookEventRequest` loses its `ReplayedBy` property (retains only `Id`). `ReplayWebhookEventHandler` gains a third constructor dependency, `ICurrentUserService` (`Anela.Heblo.Domain.Features.Users`), and resolves `entry.LastReplayedBy` internally via `_currentUserService.GetCurrentUser().Name ?? "unknown"` — the exact expression `CreateAdjustmentHandler` already uses for its `CreatedBy` audit field. `SmartsuppWebhookAuditController.Replay` drops its `User.Identity?.Name` read and sends `new ReplayWebhookEventRequest { Id = id }`. No new abstractions, no DI registration changes (`ICurrentUserService` is already registered via `UsersModule.cs`), no schema change.

**Tech Stack:** .NET 8, C#, MediatR, xUnit, FluentAssertions, Moq, EF Core (`Microsoft.EntityFrameworkCore.InMemory`).
