# Decouple OrgChart Adapter Deserialization from Application Contracts Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Stop `OrgChartService` from deserializing the external org-chart JSON directly into the Application-layer `OrgChartResponse` contract; deserialize into a new adapter-internal JSON model instead, then explicitly map it into the existing `OrgChartResponse`/`OrganizationDto`/`PositionDto`/`EmployeeDto` contracts, with no change to `IOrgChartService`, observable behavior, or error handling.

**Architecture:** Add four `internal sealed class` POCOs in a new file `Models/OrgChartJsonModel.cs` in `Anela.Heblo.Adapters.OrgChart` that mirror the external JSON shape with no dependency on `BaseResponse`. Change `OrgChartService.GetOrganizationStructureAsync` to deserialize into `OrgChartJsonModel` instead of `OrgChartResponse`, keep the existing null-check/try-catch structure pointed at the new type, and add three small `private static` mapping methods (`MapOrganization`, `MapPosition`, `MapEmployee`) that build a new `OrgChartResponse` field-by-field per the field-mapping table in `design.r1.md`. Close the happy-path coverage gap noted in the architecture review by adding one new mapping test to the existing test file before implementing (TDD).

**Tech Stack:** .NET 8, `System.Text.Json`, xUnit, FluentAssertions, Moq (existing test stack in `Anela.Heblo.Tests`).
