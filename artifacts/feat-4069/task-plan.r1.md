# Implementation Plan: Extract Invoice Import Job Name Prefix Into a Shared Constant

## Overview
Replace the duplicated `"Import faktur:"` literal in `GetRunningInvoiceImportJobsHandler` and the `[DisplayName("Import faktur: {0}")]` attributes on `IInvoiceImportService`/`InvoiceImportService` with a single shared `InvoiceImportServiceConstants` class, so the job-name filter and the Hangfire display name can never silently drift apart.
