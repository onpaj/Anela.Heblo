# Task Plan: Fix hardcoded DesiredStateId in PrintExpeditionOrderHandler's non-printable state guard

## Overview
This is a single-file, single-method bugfix confined entirely to `PrintExpeditionOrderHandler.Handle` in the Application layer, with a matching single-file test addition. There is no frontend work (per arch-review, "Skip Design: true"), no contract/DTO change, no new dependency or DI wiring, and no data model change. The fix is a ~15-line diff (remove one dictionary entry, insert one equality check, add one test). Splitting this into multiple tasks would be artificial busywork for a solo developer — it is implemented, verified, and tested as one atomic change.
