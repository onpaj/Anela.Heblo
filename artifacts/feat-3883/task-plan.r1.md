# Stop Built-In HTTP Logging From Capturing the Raw Authorization Bearer Token Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Stop ASP.NET Core's built-in `HttpLoggingMiddleware` from logging the real value of the `Authorization` request header (a live bearer token) in every environment, by removing `logging.RequestHeaders.Add("Authorization")` from `AddCrossCuttingServices`, while a new regression test locks the fix in.

**Architecture:** One-line removal in `backend/src/Anela.Heblo.API/Extensions/ServiceCollectionExtensions.cs` per the chosen architecture decision (Decision 1, Option 1 — omission over interceptor-based redaction). ASP.NET Core's `HttpLoggingMiddleware` already treats any header not explicitly present in `HttpLoggingOptions.RequestHeaders` as sensitive (its value is omitted from log output); removing the `.Add("Authorization")` call reverts `Authorization` to that same default-safe treatment, matching how `Cookie` and every other unlisted header already behaves. The fix is verified by a unit test that builds a `ServiceCollection` via the real `AddCrossCuttingServices()` extension method (mirroring the existing `CombinedPrintQueueSinkRegistrationTests.cs` pattern) and asserts on the resolved `IOptions<HttpLoggingOptions>` — fast, deterministic, and directly exercises the exact configuration being fixed, with no dependency on the full HTTP pipeline or log-provider wiring.

**Tech Stack:** .NET 8, ASP.NET Core `Microsoft.AspNetCore.HttpLogging`, xUnit, FluentAssertions.
