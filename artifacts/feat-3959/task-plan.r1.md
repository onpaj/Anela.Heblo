# Implementation Plan: Unit test coverage for `VatClassificationRule.Evaluate`

## Feature
Add a dedicated xUnit test class (`VatClassificationRuleTests`) covering `VatClassificationRule.Evaluate` in `Anela.Heblo.Domain`, bringing that file's line coverage from 0% to 100%. This is a pure test-authoring task — **no production code changes**. It requires one small, additive extension to the shared `InvoiceClassificationFixtures` test helper so the new test class can build invoices with a `CompanyVat` value, consistent with how sibling rule test classes already build invoices.

## Goal
- `backend/test/Anela.Heblo.Tests/Features/InvoiceClassification/Rules/VatClassificationRuleTests.cs` exists, compiles, and passes.
- `VatClassificationRule.cs` reaches 100% line coverage.
- `InvoiceClassificationFixtures.CreateInvoice` gains an optional `companyVat` parameter; every existing call site continues to compile and pass unchanged.
- No changes to `VatClassificationRule.cs`, `ReceivedInvoice.cs`, `IClassificationRule.cs`, or any other production code.

## Architecture / Tech Stack
- Backend: .NET 8, xUnit + FluentAssertions (already referenced by `Anela.Heblo.Tests`, no new package dependencies).
- Test project: `backend/test/Anela.Heblo.Tests/Anela.Heblo.Tests.csproj`.
- Pattern: pure in-memory AAA unit tests, no mocking, no DI, no async — mirrors the four existing sibling rule test classes in the same folder (`AmountClassificationRuleTests.cs`, `CompanyNameClassificationRuleTests.cs`, `DescriptionClassificationRuleTests.cs`, `ItemDescriptionClassificationRuleTests.cs`).

## Files touched

| File | Change |
|---|---|
| `backend/test/Anela.Heblo.Tests/Features/InvoiceClassification/TestHelpers/InvoiceClassificationFixtures.cs` | Modified — add optional `string companyVat = ""` parameter to `CreateInvoice`, inserted immediately before the trailing `params string[] itemNames` (the only legal position in C#), and set it on the constructed `ReceivedInvoice`. |
| `backend/test/Anela.Heblo.Tests/Features/InvoiceClassification/Rules/VatClassificationRuleTests.cs` | New — xUnit test class covering `VatClassificationRule.Evaluate` per FR-3 through FR-6 of the spec. |

## Reference: system under test (unchanged, quoted for exactness)
...
`ReceivedInvoice.CompanyVat` (`backend/src/Anela.Heblo.Domain/Features/InvoiceClassification/ReceivedInvoice.cs`, line 9):
```csharp
public string CompanyVat { get; set; } = string.Empty;
```

Current fixture (`backend/test/Anela.Heblo.Tests/Features/InvoiceClassification/TestHelpers/InvoiceClassificationFixtures.cs`), before this plan's changes:
```csharp
...
Confirmed via repo-wide grep (per arch review) that every existing call to `CreateInvoice` across the test suite uses **named** arguments — so inserting a new optional parameter ahead of `itemNames` cannot break any existing positional call.

## Test commands (repo-confirmed, from `docs/architecture/testing-strategy.md`)
