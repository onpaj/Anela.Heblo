# CreateJournalTagHandler Unit Tests Implementation Plan

**Goal:** Add a unit test suite for `CreateJournalTagHandler` covering the unauthenticated-rejection branch and the authenticated-success/persistence branch, closing the existing coverage gap with no production code changes.

**Architecture:** A single new xUnit test class, `CreateJournalTagHandlerTests`, is added to `backend/test/Anela.Heblo.Tests/Features/Journal/`, mirroring the existing sibling `CreateJournalEntryHandlerTests.cs` in the same directory field-for-field: constructor-wired `Mock<IJournalTagRepository>`, `Mock<ICurrentUserService>`, and `Mock<ILogger<CreateJournalTagHandler>>`, with the handler invoked directly (bypassing the MediatR pipeline). Three `[Fact]` test methods cover the two guard-condition sub-cases and the success/persistence path.
