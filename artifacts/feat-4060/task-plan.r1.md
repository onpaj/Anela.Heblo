# Bank ImportTab errorType Refactor Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make `ImportTab.tsx`'s bank-statement status badge branch on the DTO's `errorType` field instead of hardcoding the literal `"OK"`, with zero visible behavior change.

**Architecture:** Single-file frontend refactor. `getImportStatusIcon` currently compares `importResult === "OK"`; it will instead branch on `errorType` nullness (`errorType` is `null`/`undefined` on success, the error string otherwise, per `BankStatementImportDto.ErrorType` on the backend, already generated into the TS client). The `importResult` parameter is dropped from the function; the single call site is updated to pass `statement.errorType` instead. No backend, contract, or generated-client changes.

**Tech Stack:** React + TypeScript (CRA / `react-scripts`), Jest + React Testing Library, lucide-react icons.
