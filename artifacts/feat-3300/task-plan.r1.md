# Eliminate N+1 Graph API Calls in GetAppRoleMembersAsync Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the per-user serial HTTP loop in `GraphService.GetAppRoleMembersAsync` Step 5 with Graph `$batch` requests, reducing O(N) sequential calls to ⌈N/20⌉ batch calls.

**Architecture:** Only Step 5 of `GetAppRoleMembersAsync` changes — the per-user `foreach` loop is replaced with a chunked batch loop that POSTs up to 20 sub-requests per call to `POST /v1.0/$batch`. The same `graphToken` and `httpClient` already in scope are reused. All other steps (token acquisition, SP lookup, role ID resolution, assignment pagination, group expansion, caching) are untouched.

**Tech Stack:** .NET 8, `System.Text.Json` (already used in the file), `Microsoft.Graph` batch endpoint (`POST https://graph.microsoft.com/v1.0/$batch`), xUnit + FluentAssertions + Moq (existing test stack).
