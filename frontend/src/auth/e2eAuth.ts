...
export const isE2ETestMode = (): boolean => {
  ...
  const hasE2EAuth = cookies.includes('E2ETestSession=') || cookies.includes('e2e-auth=') || cookies.includes('E2ETestAuth=');
  
  // Check if we're accessing the app via the E2E endpoint or parameter
  const isE2EUrl = window.location.href.includes('/api/e2etest/app') || 
...
export const getE2EAccessToken = (): string | null => {
  ...
  if (e2eToken) {
    console.log('🎫 E2E auth using Service Principal token from sessionStorage');
    return e2eToken;
  }
  
  console.log('🎫 E2E auth: No token found in sessionStorage, using session cookies (no Authorization header)');
  return null;
};
...
export const useE2EAuth = () => {
  ...
  useEffect(() => {
    const initializeE2EAuth = async () => {
      ...
      if (isE2ETestMode()) {
        console.log('✅ E2E test mode detected - auto-authenticating with test user');
