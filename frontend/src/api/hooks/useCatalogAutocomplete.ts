...
export function useCatalogAutocomplete(searchTerm?: string, limit = 20, productTypes?: ProductType[]) {
  return useQuery({
    queryKey: [...QUERY_KEYS.catalog, 'autocomplete', searchTerm, limit, productTypes],
    queryFn: async () => {
      ...
      const response = await apiClient.catalog_GetProductsForAutocomplete(searchTerm, limit, productTypes);
