...
const CreateManufactureOrderModal: React.FC<CreateManufactureOrderModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-graphite-surface rounded-lg shadow-xl dark:shadow-soft-dark max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-graphite-border">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-graphite-text flex items-center gap-2">
            <Package className="h-5 w-5 text-indigo-600 dark:text-graphite-accent" />
        ...
        <div className="p-6">
          {/* Batch Summary */}
          <div className="bg-gray-50 dark:bg-graphite-surface-2 rounded-lg p-4 mb-6">
            <h3 className="text-sm font-medium text-gray-900 dark:text-graphite-text mb-3">Shrnutí dávky</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              <div>
                <span className="text-gray-500 dark:text-graphite-muted">Produkt:</span>
                <div className="font-medium">{batchResult.productName}</div>
                <div className="text-gray-500 dark:text-graphite-muted font-mono text-xs">{batchResult.productCode}</div>
              </div>
              <div>
                <span className="text-gray-500 dark:text-graphite-muted">Původní dávka:</span>
                <div className="font-medium">{batchResult.originalBatchSize}g</div>
              </div>
              <div>
                <span className="text-gray-500 dark:text-graphite-muted">Nová dávka:</span>
                <div className="font-medium text-indigo-600 dark:text-graphite-accent">{batchResult.newBatchSize}g</div>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Error Message */}
            {error && (
              <div className="bg-red-50 border border-red-200 dark:bg-red-900/20 dark:border-red-900/40 rounded-md p-4 flex items-start gap-3">
            ...
            <div>
              ...
              <input
                ...
                required
                disabled={isLoading}
                className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 disabled:opacity-50 dark:bg-graphite-surface-2 dark:border-graphite-border dark:text-graphite-text dark:placeholder-graphite-faint"
              />
            ...
            {/* Products Summary */}
            <div>
              <h4 className="text-sm font-medium text-gray-900 dark:text-graphite-text mb-3">
                Finální produkty ({batchResult.ingredients?.length || 0})
              </h4>
              <div className="bg-gray-50 dark:bg-graphite-surface-2 rounded-md p-3 max-h-40 overflow-y-auto">
                <div className="space-y-2">
                  {batchResult.ingredients?.map((ingredient, index) => (
                    <div key={ingredient.productCode} className="flex justify-between text-sm">
                      <span className="text-gray-700 dark:text-graphite-muted">{ingredient.productName}</span>
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-200 bg-gray-50 dark:border-graphite-border dark:bg-graphite-surface-2">
          <button
            type="button"
            onClick={handleClose}
            disabled={isLoading}
            className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 disabled:opacity-50 dark:text-graphite-muted dark:bg-graphite-surface dark:border-graphite-border dark:hover:bg-white/5"
          >
            Zrušit
          </button>
          <button
