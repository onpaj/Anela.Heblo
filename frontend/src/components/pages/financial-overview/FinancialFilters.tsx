...
export const FinancialFilters: React.FC<FinancialFiltersProps> = ({
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center">
            ...
            <label
              ...
              className="ml-2 block text-sm text-gray-900 dark:text-graphite-text flex items-center"
            >
    ...
    return (
      <div className="mb-6">
        <button
          ...
          className="w-full flex items-center justify-between px-4 py-2 bg-white dark:bg-graphite-surface shadow dark:shadow-soft-dark rounded-lg text-left"
        >
          <span className="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-graphite-muted">
            <SlidersHorizontal className="w-4 h-4" />
            Filtry & období — {getPeriodLabel(selectedPeriod)}
          </span>
          {isExpanded ? (
            <ChevronUp className="w-4 h-4 text-gray-500 dark:text-graphite-muted" />
          ) : (
            <ChevronDown className="w-4 h-4 text-gray-500 dark:text-graphite-muted" />
          )}
        </button>
        {isExpanded && (
          <div className="mt-3 p-4 bg-white dark:bg-graphite-surface shadow dark:shadow-soft-dark rounded-lg">
            {controlsBlock}
          </div>
        )}
        {isRefetching && (
          <div className="flex items-center text-sm text-gray-500 dark:text-graphite-muted mt-2">
  ...
  return (
    <div className="mb-6 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
      {controlsBlock}
      {isRefetching && (
        <div className="flex items-center text-sm text-gray-500 dark:text-graphite-muted">
          <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-indigo-600 mr-2" />
          Aktualizuji data...
        </div>
      )}
