...
const ManufacturingStockAnalysis: React.FC = () => {
  ...
  const columns = useMemo((): GridColumn<ManufacturingStockItemDto>[] => [
    {
      id: 'product',
      header: 'Produkt',
      canHide: false,
    ...
    },
    {
      ...
      renderCell: (item) => (
        ...
        >
          {isInPlanningList(item.code) ? <Check className="h-4 w-4" /> : <Plus className="h-4 w-4" />}
        </button>
      ),
    },
    {
    ...
    },
    {
      id: 'quarantine',
      header: 'Karant.',
      sortBy: ManufacturingStockSortBy.Quarantine,
      align: 'right',
      minWidth: 60,
      defaultWidth: 120,
      cellClassName: 'text-xs text-gray-900',
    ...
    },
    {
      id: 'planned',
      header: 'Plán',
      sortBy: ManufacturingStockSortBy.Planned,
      align: 'right',
      minWidth: 60,
      defaultWidth: 120,
      cellClassName: 'text-xs text-gray-900',
    ...
    },
    {
      id: 'salesInPeriod',
      header: 'Prodeje',
      sortBy: ManufacturingStockSortBy.SalesInPeriod,
      align: 'right',
      minWidth: 60,
      defaultWidth: 144,
      cellClassName: 'text-xs text-gray-900',
    ...
    },
    {
      id: 'dailySales',
      header: 'Prod/den',
      sortBy: ManufacturingStockSortBy.DailySales,
      align: 'right',
      minWidth: 100,
      defaultWidth: 144,
      cellClassName: 'text-xs text-gray-900',
    ...
    {
      id: 'stockDaysAvailable',
      header: 'NS',
      sortBy: ManufacturingStockSortBy.StockDaysAvailable,
      align: 'right',
      minWidth: 90,
      defaultWidth: 120,
      cellClassName: 'text-xs text-gray-900',
    ...
    },
    {
      id: 'minimumStock',
      header: 'Min',
      sortBy: ManufacturingStockSortBy.MinimumStock,
      align: 'right',
      minWidth: 90,
      defaultWidth: 120,
      cellClassName: 'text-xs text-gray-900',
    ...
    },
    {
      id: 'optimalDaysSetup',
      header: 'Nastavení',
      sortBy: ManufacturingStockSortBy.OptimalDaysSetup,
      align: 'right',
      minWidth: 90,
      defaultWidth: 120,
      cellClassName: 'text-xs text-gray-900',
    ...
    },
    {
      id: 'batchSize',
      header: 'ks/šarže',
      sortBy: ManufacturingStockSortBy.BatchSize,
      align: 'right',
      minWidth: 80,
      defaultWidth: 96,
      cellClassName: 'text-xs text-gray-900',
  ...
  const ProductFamilySubgrid: React.FC<{
    productFamily: string;
    isLoading: boolean;
  }> = ({ productFamily, isLoading }) => {
      ...
      <>
        {items.map((subItem) => (
          ...
          >
            ...
            >
              <div className="font-medium">
                {subItem.stockDaysAvailable > 999
                  ? "∞"
                  : formatNumber(subItem.stockDaysAvailable, 0)}
              </div>
            </td>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start">
                      ...
                      <div>
                        ...
                        Skladem &lt; minimální zásoba
                      </div>
                    </div>
                    <div className="flex items-start">
                      <div className="w-3 h-3 bg-gray-200 rounded-sm mr-2 mt-0.5 flex-shrink-0"></div>
                      <div>
                        <span className="font-medium text-gray-200">Šedé:</span>{" "}
                        Nezkonfigurováno (chybí OptimalStockDaysSetup)
                      </div>
                    </div>
                    <div className="flex items-start">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value={TimePeriodFilter.FutureQuarter}>
                      Budoucí kvartal
                    </option>
                    <option value={TimePeriodFilter.Y2Y}>
                      Y2Y (12 měsíců)
                    </option>
                  ...
                  <>
                    <div>
                      ...
                      <input
                        type="date"
                        value={
                          filters.customFromDate?.toISOString().split("T")[0] ||
                          ""
                        }
                        onChange={(e) =>
                          handleFilterChange({
                    ...
                    <div>
                      ...
                      <input
                        type="date"
                        value={
                          filters.customToDate?.toISOString().split("T")[0] ||
                          ""
                        }
                        onChange={(e) =>
                          handleFilterChange({
            ...
            >
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {tableData.map((item) => {
                  const hasSubItems = shouldShowExpandButton(item);
                  const isExpanded =
                    item.productFamily && expandedRows.has(item.productFamily);
                  const isLoading = !!(
                    item.productFamily &&
                    loadingSubgrids.has(item.productFamily)
                  );
                    ...
                    <React.Fragment key={item.code}>
                      ...
                      >
                        {orderedColumns.map((col) => {
                          const colState = columnState.find((s) => s.id === col.id);
                          const width = colState?.width ?? col.defaultWidth;
                          return (
                            <td
                              key={col.id}
