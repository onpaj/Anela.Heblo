...
import { PurchaseOrderLineDto, ContactVia, SupplierDto } from '../../api/generated/api-client';
import { MaterialForPurchaseDto } from '../../api/hooks/useMaterials';
import { 
  PurchaseOrderFormProps, 
  FormData 
} from '../purchase-orders/form/PurchaseOrderTypes';
import { validateForm, clearFieldError } from '../purchase-orders/form/PurchaseOrderValidation';
import { 
  ...
  const updateLine = (index: number, field: keyof PurchaseOrderLineDto, value: string | number) => {
    setFormData(prev => {
      ...
      if (field === 'quantity' || field === 'unitPrice') {
        newLines[index].lineTotal = calculateLineTotal(newLines[index].quantity || 0, newLines[index].unitPrice || 0);
      }
