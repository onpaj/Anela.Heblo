...
const FinancialOverview: React.FC = () => {
  const [selectedPeriod, setSelectedPeriod] = useState<PeriodType>('current-year');
  ...
  const { data, isLoading, error, isRefetching } = useFinancialOverviewQuery(months, includeStockData);
  ...
  const chartData = React.useMemo(() => {
    ...
    const stockChangeData = sortedData.map(item => item.totalStockValueChange || 0);
    const totalBalanceData = sortedData.map(item => item.totalBalance || item.financialBalance);
      ...
      <div className="w-full max-w-none px-4 sm:px-6 lg:px-8">
        <div className="mb-8 p-4 bg-red-50 border border-red-200 rounded-lg">
          <div className="flex items-center">
            ...
            <h3 className="text-red-800 font-medium">Chyba při načítání finančních dat</h3>
          </div>
    ...
    <div className="w-full max-w-none">
      ...
      <div className="mb-6 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div>
            <label htmlFor="stock-toggle" className="block text-sm font-medium text-gray-700 mb-2">
              Zobrazení dat:
            </label>
            <div className="flex items-center">
              <input
              ...
              />
              <label htmlFor="stock-toggle" className="ml-2 block text-sm text-gray-900 flex items-center">
      ...
      {/* Summary Cards */}
      {data?.summary && (
        <div className={`grid grid-cols-1 sm:grid-cols-2 ${includeStockData ? 'xl:grid-cols-6' : 'lg:grid-cols-4'} gap-4 mb-6`}>
          {/* Total Income */}
          ...
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="p-3">
              <div className="flex items-center">
                <div className="flex-shrink-0">
                  <DollarSign className={`h-5 w-5 ${data.summary.totalBalance >= 0 ? 'text-emerald-400' : 'text-red-400'}`} />
                </div>
                <div className="ml-3 w-0 flex-1">
                  <dl>
                    <dt className="text-xs font-medium text-gray-500 truncate">
                    ...
                    <dd className={`text-sm font-medium ${data.summary.totalBalance >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
          ...
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="p-3">
              <div className="flex items-center">
                <div className="flex-shrink-0">
                  <Calendar className={`h-5 w-5 ${data.summary.averageMonthlyBalance >= 0 ? 'text-blue-400' : 'text-red-400'}`} />
                </div>
                <div className="ml-3 w-0 flex-1">
                  <dl>
                    <dt className="text-xs font-medium text-gray-500 truncate">
                    ...
                    <dd className={`text-sm font-medium ${data.summary.averageMonthlyBalance >= 0 ? 'text-blue-600' : 'text-red-600'}`}>
            ...
            <>
              ...
              <div className="bg-white overflow-hidden shadow rounded-lg">
                <div className="p-3">
                  <div className="flex items-center">
                    <div className="flex-shrink-0">
                      <Package className={`h-5 w-5 ${data.summary.stockSummary.totalStockValueChange && data.summary.stockSummary.totalStockValueChange >= 0 ? 'text-purple-400' : 'text-orange-400'}`} />
                    </div>
                    <div className="ml-3 w-0 flex-1">
                      <dl>
                        <dt className="text-xs font-medium text-gray-500 truncate">
                        ...
                        <dd className={`text-sm font-medium ${data.summary.stockSummary.totalStockValueChange && data.summary.stockSummary.totalStockValueChange >= 0 ? 'text-purple-600' : 'text-orange-600'}`}>
                          {formatCurrency(data.summary.stockSummary.totalStockValueChange || 0)}
                        </dd>
                      </dl>
                    </div>
                  </div>
              ...
              <div className="bg-white overflow-hidden shadow rounded-lg">
                <div className="p-3">
                  <div className="flex items-center">
                    <div className="flex-shrink-0">
                      <BarChart3 className={`h-5 w-5 ${data.summary.stockSummary.totalBalanceWithStock && data.summary.stockSummary.totalBalanceWithStock >= 0 ? 'text-emerald-400' : 'text-red-400'}`} />
                    </div>
                    <div className="ml-3 w-0 flex-1">
                      <dl>
                        <dt className="text-xs font-medium text-gray-500 truncate">
                        ...
                        <dd className={`text-sm font-medium ${data.summary.stockSummary.totalBalanceWithStock && data.summary.stockSummary.totalBalanceWithStock >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                          {formatCurrency(data.summary.stockSummary.totalBalanceWithStock || 0)}
                        </dd>
                      </dl>
                    </div>
                  </div>
                  ...
                  <tr key={`${item.year}-${item.month}`} className="hover:bg-gray-50">
                    ...
                    {includeStockData && (
                      <>
                        ...
                        <td className={`px-6 py-4 whitespace-nowrap text-sm text-right font-medium ${
                          (item.totalBalance || item.financialBalance) >= 0 ? 'text-emerald-600' : 'text-red-600'
                        }`}>
