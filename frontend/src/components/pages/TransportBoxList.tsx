...
const TransportBoxList: React.FC = () => {
  ...
  const { data, isLoading, error, refetch } =
    useTransportBoxesQuery(queryRequest);
  ...
  const handleOpenNewBox = async () => {
    try {
      ...
      const { CreateNewTransportBoxRequest } = await import(
      ...
      const response =
        await apiClient.transportBox_CreateNewTransportBox(request);

      if (
        response.success &&
        response.transportBox &&
        response.transportBox.id
      ) {
