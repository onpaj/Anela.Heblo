...
const mockUseRecentAuditLogs = useRecentAuditLogs as jest.MockedFunction<
  typeof useRecentAuditLogs
>;
const mockUseRecentAuditSummary = useRecentAuditSummary as jest.MockedFunction<
  typeof useRecentAuditSummary
>;
const mockUseManualCatalogRefresh =
  useManualCatalogRefresh as jest.MockedFunction<
    typeof useManualCatalogRefresh
  >;
...
describe("Dashboard", () => {
  ...
  describe("Manual Refresh Tab", () => {
    ...
    it("should call refresh function when manufacture difficulty button is clicked", async () => {
      ...
      const manufactureDifficultyButton =
        refreshButtons[refreshButtons.length - 1];
      ...
      await waitFor(() => {
        expect(mockMutateAsync).toHaveBeenCalledWith(
          "catalog_RefreshManufactureDifficultyData",
        );
      });
    });
    ...
    it("should show success message when refresh completes successfully", async () => {
      ...
      expect(
        screen.getByText("Operace byla dokončena úspěšně."),
      ).toBeInTheDocument();
    });
  ...
  describe("Tab Navigation", () => {
    ...
    it("should switch between tabs correctly", async () => {
      ...
      expect(
        screen.getByText("Poslední audit logy (24 hodin)"),
      ).toBeInTheDocument();
