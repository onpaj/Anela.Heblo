...
describe("TransportBoxDetail", () => {
  ...
  it("should render transport box details correctly", () => {
    ...
    render(<TransportBoxDetail boxId={1} isOpen={true} onClose={jest.fn()} />, {
  ...
  it("should display error state", () => {
    ...
    expect(
      screen.getByText("Chyba při načítání detailu boxu"),
    ).toBeInTheDocument();
  });
  ...
  describe("State transition buttons", () => {
    it("should show next state button for New state", () => {
      ...
      render(
        <TransportBoxDetail boxId={1} isOpen={true} onClose={jest.fn()} />,
        { wrapper: createWrapper },
      );
    ...
    it("should show only previous button for Closed state", () => {
      ...
      const stateTransitionButtons = allButtons.filter(
        (btn) =>
          btn.textContent &&
          [
            "Nový",
            ...
            "Naskladněný",
            "V rezervě",
          ].some((state) => btn.textContent!.includes(state)),
      );
  ...
  describe("Error handling", () => {
    it("should display mutation error", () => {
      ...
      expect(mockUseChangeTransportBoxState().error).toEqual(
