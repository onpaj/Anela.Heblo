...
const ManufactureOutput: React.FC = () => {
  ...
  const [selectedMonth, setSelectedMonth] = useState<ManufactureOutputMonth | null>(null);
  ...
  const chartData = useMemo(() => {
    ...
    data.months.forEach(month => {
      month.products.forEach(product => {
        const existing = productTotals.get(product.productCode) || { name: product.productName, total: 0 };
    ...
    topProducts.forEach((product, index) => {
      productColorMap.set(product[0], PRODUCT_COLORS[index % PRODUCT_COLORS.length]);
    });
    ...
    [...topProducts].reverse().forEach(([productCode, productInfo]) => {
      ...
      datasets.push({
        ...
        data: data.months.map(month => {
          const product = month.products.find(p => p.productCode === productCode);
      ...
      tooltip: {
        callbacks: {
          ...
          afterLabel: (context: any) => {
            ...
            const product = monthData.products.find(p => p.productName === datasetLabel);
            if (product) {
              return [
  ...
  const summaryStats = useMemo(() => {
    ...
    const totalOutput = data.months.reduce((sum, month) => sum + month.totalOutput, 0);
    ...
    const maxMonth = data.months.reduce((max, month) => 
      month.totalOutput > max.totalOutput ? month : max
    , data.months[0]);
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-red-600">Chyba při načítání dat: {(error as Error).message}</div>
      </div>
    );
  }
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex-shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-2xl font-semibold text-gray-900">Přehled výroby</h1>
          </div>
          <button
            onClick={() => refetch()}
