...
const IndexRootsTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {roots.length === 0 ? (
        <p className="text-sm text-gray-500 dark:text-graphite-muted">Žádné kořeny nejsou nakonfigurovány.</p>
      ) : (
                    ...
                    <span
                      className={`px-1.5 py-0.5 rounded text-xs ${
                        root.isActive
                          ...
                          : "bg-gray-100 dark:bg-graphite-surface-2 text-gray-500 dark:text-graphite-muted"
                      }`}
                    >
      ...
      <form onSubmit={handleSubmit} className="space-y-3 border-t border-gray-200 dark:border-graphite-border pt-4">
        <h3 className="text-sm font-semibold text-gray-700 dark:text-graphite-muted">Přidat kořen</h3>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label htmlFor="folderPath" className="block text-xs text-gray-500 dark:text-graphite-muted mb-1">Cesta *</label>
            <input
              id="folderPath"
              type="text"
              value={folderPath}
          ...
          <div>
            <label htmlFor="displayName" className="block text-xs text-gray-500 dark:text-graphite-muted mb-1">Název (volitelný)</label>
            <input
              id="displayName"
              type="text"
              value={displayName}
          ...
          <div>
            <label htmlFor="driveId" className="block text-xs text-gray-500 dark:text-graphite-muted mb-1">Drive ID *</label>
            <input
              id="driveId"
              type="text"
              value={driveId}
