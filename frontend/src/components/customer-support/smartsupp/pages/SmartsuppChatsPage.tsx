...
const SmartsuppChatsPage: React.FC = () => {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
      <div
        className={`${mobileView !== "list" ? "hidden md:flex" : "flex"} items-center justify-end px-4 py-2 border-b border-gray-200 dark:border-graphite-border bg-white dark:bg-graphite-surface`}
      >
        <button
          type="button"
          onClick={() => queryClient.invalidateQueries({ queryKey: QUERY_KEYS.smartsupp })}
          disabled={isFetching}
          className="inline-flex items-center gap-2 px-3 py-1.5 text-sm font-medium rounded-md border border-gray-300 dark:border-graphite-border bg-white dark:bg-graphite-surface dark:text-graphite-text hover:bg-gray-50 dark:hover:bg-white/5 disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {isFetching ? "Načítám…" : "Obnovit"}
        </button>
      </div>

      <div className="flex flex-1 overflow-hidden bg-white dark:bg-graphite-surface">
        <div
        ...
        >
          {selectedConversation ? (
            ...
            <div className="flex items-center justify-center h-full text-gray-400 dark:text-graphite-faint text-sm">
              Vyberte konverzaci
            </div>
          )}
        </div>
        ...
        {mobileView === "contact" && selectedConversation && (
          ...
          >
            <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-200 dark:border-graphite-border bg-white dark:bg-graphite-surface">
              <button
                type="button"
                data-testid="back-to-chat-btn"
                onClick={() => setMobileView("chat")}
                aria-label="Zpět na konverzaci"
                className="flex items-center justify-center min-h-[40px] min-w-[40px] p-1 -ml-1 text-gray-600 dark:text-graphite-muted flex-shrink-0"
              >
                <ArrowLeft className="w-5 h-5" />
              </button>
              <span className="font-semibold text-gray-900 dark:text-graphite-text">Detail kontaktu</span>
            </div>
