...
const TransportBoxActions: React.FC<TransportBoxActionsProps> = ({
  ...
  const previousTransitions =
    transportBox.allowedTransitions?.filter(
      (transition) =>
            ...
            {previousTransitions.length > 0 ? (
              previousTransitions.map((transition, index) => (
                <button
                  ...
                  title={
                    transition.systemOnly
                      ? "Pouze systémový přechod"
                      : `Změnit na: ${stateLabels[transition.newState!] || transition.newState}`
                  }
                >
                  <div className="flex items-center">
                    ...
                    <span>
                      {stateLabels[transition.newState!] || transition.newState}
                    </span>
                  </div>
                </button>
              ))
            ...
            >
              <div className="flex items-center">
                ...
                <span className="text-sm font-medium">
                  {stateLabels[transportBox.state || ""] || transportBox.state}
                </span>
              </div>
            </div>
          </div>
                  ...
                  className={`flex-1 flex items-center justify-center px-3 rounded-lg transition-colors text-sm font-medium ${
                    transition.systemOnly
                      ...
                      : transition.newState === "InTransit" ||
                        ...
                        : transition.newState === "Opened" ||
                            transition.newState === "New"
