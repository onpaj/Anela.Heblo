...
export const createDefaultLine = (): PurchaseOrderLineDto & {
...
export const transformExistingOrderData = (
  existingOrderData: any,
): FormData => {
  const linesWithMaterials =
    existingOrderData.lines && existingOrderData.lines.length > 0
      ? existingOrderData.lines.map((line: any) =>
  ...
  return {
    ...
    orderDate: existingOrderData.orderDate
      ? new Date(existingOrderData.orderDate).toISOString().split("T")[0]
      : new Date().toISOString().split("T")[0],
    expectedDeliveryDate: existingOrderData.expectedDeliveryDate
      ? new Date(existingOrderData.expectedDeliveryDate)
          .toISOString()
          .split("T")[0]
      : "",
