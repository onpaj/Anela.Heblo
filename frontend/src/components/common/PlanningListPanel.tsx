...
import { X, List, Package } from "lucide-react";
import { usePlanningList } from "../../contexts/PlanningListContext";

interface PlanningListPanelProps {
  isVisible: boolean;
  onItemClick?: (item: { productCode: string; productName: string }) => void;
}

const PlanningListPanel: React.FC<PlanningListPanelProps> = ({
  isVisible,
  onItemClick,
}) => {
  const { items, removeItem, hasItems } = usePlanningList();
...
export default PlanningListPanel;