...
import {
  useLeafletDocumentsQuery,
  useLeafletContentTypesQuery,
  useDeleteLeafletDocumentMutation,
  LeafletDocumentSummary,
} from '../../api/hooks/useLeaflet';
...
const LeafletDocumentsTab: React.FC<Props> = ({ canDelete }) => {
  const [searchParams, setSearchParams] = useSearchParams();

  const getInitialPage = () => {
    const p = searchParams.get('lfPage');
    return p ? Math.max(1, parseInt(p, 10)) : DEFAULT_PAGE;
  };
  const getInitialPageSize = () => {
    const ps = searchParams.get('lfPageSize');
    const val = ps ? parseInt(ps, 10) : DEFAULT_PAGE_SIZE;
    return [10, 20, 50].includes(val) ? val : DEFAULT_PAGE_SIZE;
  };
  const getInitialSortBy = () => searchParams.get('lfSortBy') || DEFAULT_SORT_BY;
  const getInitialSortDesc = () => searchParams.get('lfSortDesc') !== 'false';
  const getInitialFilenameFilter = () => searchParams.get('lfFilename') || '';
  const getInitialStatusFilter = () => searchParams.get('lfStatus') || '';
  const getInitialContentTypeFilter = () => searchParams.get('lfContentType') || '';
  ...
  const { data: contentTypesData } = useLeafletContentTypesQuery();
  const deleteDocument = useDeleteLeafletDocumentMutation();

  // Sync state → URL
  useEffect(() => {
    const params = new URLSearchParams(searchParams);

    if (pageNumber !== DEFAULT_PAGE) params.set('lfPage', pageNumber.toString());
    else params.delete('lfPage');

    if (pageSize !== DEFAULT_PAGE_SIZE) params.set('lfPageSize', pageSize.toString());
    else params.delete('lfPageSize');

    if (sortBy !== DEFAULT_SORT_BY) params.set('lfSortBy', sortBy);
    else params.delete('lfSortBy');

    if (sortDescending !== DEFAULT_SORT_DESC) params.set('lfSortDesc', 'false');
    else params.delete('lfSortDesc');

    if (filenameFilter) params.set('lfFilename', filenameFilter);
    else params.delete('lfFilename');

    if (statusFilter) params.set('lfStatus', statusFilter);
    else params.delete('lfStatus');

    if (contentTypeFilter) params.set('lfContentType', contentTypeFilter);
    else params.delete('lfContentType');
  ...
  useEffect(() => {
    const newPage = Math.max(1, parseInt(searchParams.get('lfPage') || '1', 10));
    const newPageSizeRaw = parseInt(searchParams.get('lfPageSize') || '20', 10);
    const newPageSize = [10, 20, 50].includes(newPageSizeRaw) ? newPageSizeRaw : DEFAULT_PAGE_SIZE;
    const newSortBy = searchParams.get('lfSortBy') || DEFAULT_SORT_BY;
    const newSortDesc = searchParams.get('lfSortDesc') !== 'false';
    const newFilename = searchParams.get('lfFilename') || '';
    const newStatus = searchParams.get('lfStatus') || '';
    const newContentType = searchParams.get('lfContentType') || '';
...
export default LeafletDocumentsTab;
